...
export default function Sidebar({ currentPage, onNavigate, isOpen }) {
  ...
  const manualReviewCount = mockCases.filter((c) => c.status === 'MANUAL_REVIEW').length;
  const taskforceMissionCount = mockCases.filter(
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-t border-slate-800 space-y-3">
        ...
        <p className="text-xs text-slate-500">v4.0 — Tencent Cloud</p>
      </div>
    </aside>
  );
}
