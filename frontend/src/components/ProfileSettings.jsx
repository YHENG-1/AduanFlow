...
// No parameter needed — SETTINGS_KEY is sufficient to locate the record
...
export default function ProfileSettings() {
  ...
  // Lazy initialisers: loadSettings() is called only once on mount
  const [displayName, setDisplayName] = useState(() => loadSettings()?.displayName ?? user?.name ?? '');
  const [checklistState, setChecklistState] = useState(
    () => loadSettings()?.checklistState ?? Object.fromEntries(CHECKLIST.map((c) => [c.key, false]))
  );
  ...
  const [emailEnabled, setEmailEnabled] = useState(() => loadSettings()?.emailEnabled ?? true);
  const [quietHours, setQuietHours] = useState(() => loadSettings()?.quietHours ?? false);
  const [security, setSecurity] = useState(() => loadSettings()?.security ?? { new_device_alert: true, session_timeout: '30' });
  const [saved, setSaved] = useState(false);

  const handleSave = () => {
    localStorage.setItem(
      SETTINGS_KEY,
      JSON.stringify({ displayName, checklistState, notifs, emailEnabled, quietHours, security })
    );
    setSaved(true);
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input id="pwd-current" type="password" placeholder="••••••••" className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors bg-white" />
                  </div>
                  <div>
                    <label htmlFor="pwd-new" className="block text-xs font-medium text-slate-500 mb-1">New Password</label>
                    <input id="pwd-new" type="password" placeholder="••••••••" className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors bg-white" />
                  </div>
                  <div>
                    <label htmlFor="pwd-confirm" className="block text-xs font-medium text-slate-500 mb-1">Confirm New Password</label>
                    <input id="pwd-confirm" type="password" placeholder="••••••••" className="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors bg-white" />
                  </div>
                  <button className="text-xs text-blue-600 hover:text-blue-700 font-semibold">Update password</button>
                </div>
              </div>
...
function GmailIntegrationSection() {
  ...
  const [clientId, setClientId] = React.useState('1041907708486-uvplue4dp8pl64bre8a36u0qs5vc8lsn.apps.googleusercontent.com');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="1041907708486-uvplue4dp8pl64bre8a36u0qs5vc8lsn.apps.googleusercontent.com"
              className="w-full px-3.5 py-2 text-xs border border-slate-200 rounded-lg font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            />
          </div>
          <div>
        ...
        <div className="flex gap-3 pt-2">
          <button
            ...
            onClick={() => {
              ...
              window.location.href = `${BACKEND_URL}/api/auth/google/login?client_id=${encodeURIComponent(clientId || '')}`;
            }}
