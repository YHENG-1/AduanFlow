...
function getStepStatus(stepKey, caseData) {
  ...
  const stepMap = {
    ...
    security: ['pii masked', 'security agent', 'account freeze', 'account frozen'],
    classification: ['case classified'],
    verification: ['verification pass', 'verification fail', 'verification manual_review'],
    financial: ['provisional credit', 'reversal executed', 'auto-reversal executed'],
    communication: ['final response', 'acknowledgement sent'],
  };
...
export default function CaseDetail({ caseData, onBack }) {
  ...
  const cat = DISPUTE_CATEGORIES[caseData.category];
  const verifColors = {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-card p-4">
          ...
          <p className="text-sm font-semibold text-slate-800 mt-0.5">
            {new Date(caseData.dueDate).toLocaleDateString('en-MY', { day: 'numeric', month: 'short' })}
          </p>
