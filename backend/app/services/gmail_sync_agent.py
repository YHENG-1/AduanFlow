    ...
    def get_decrypted_credentials(self, session: Session) -> dict:
        ...
        try:
            ...
            return {
                ...
                "client_id": encryption_service.decrypt(settings_obj.gmail_client_id_encrypted) if settings_obj.gmail_client_id_encrypted else None,
                "client_secret": encryption_service.decrypt(settings_obj.gmail_client_secret_encrypted) if settings_obj.gmail_client_secret_encrypted else None,
            }
    ...
    def run_sync_cycle(self) -> dict:
        ...
        try:
            with Session(engine) as session:
                ...
                raw_client_id = os.getenv("GOOGLE_CLIENT_ID") or creds.get("client_id") or "1041907708486-uvplue4dp8pl64bre8a36u0qs5vc8lsn.apps.googleusercontent.com"
            ...
            with Session(engine) as session:
                for uid in recent_ids:
                    try:
                        ...
                        session.add(case_obj)
                        synced_cases.append(case_obj.id)
                        if intake.get("used_tools"):
                            logger.info(f"[{self.agent_name}] Rhea agent called tools: {intake['used_tools']} for {case_obj.id}")
