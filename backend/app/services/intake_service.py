...
class IntakeService:
    def process_incoming_complaint(
        ...
        """
        Processes complaint intake:
        1. Parse OCR fields / text
        2. Encrypt NRIC, account number, card number, dispute amount at rest
        3. Classify category & urgency according to BNM SLA guidelines
        4. Run MCP Core Banking verification
        5. Execute autonomous financial resolution if PASS
        6. Draft compliant customer email
        """
        ...
        ocr_results = {
            "confidence": 0.94 if attachment_name else 0.89,
            "extractedFields": {
        ...
        audit_log = [
            {"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Email MCP", "action": "Complaint email received", "detail": f"From {customer_email}"},
            {"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Intake Agent", "action": "OCR extraction completed", "detail": f"Extracted {customer_name}, {masked_acc}"},
            {"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Security Agent", "action": "PII encrypted at rest", "detail": "NRIC, Account, Amount encrypted"},
            {"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Classification Agent", "action": "Case classified", "detail": f"{category}, Urgency: {urgency}"},
            {"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Verification Agent", "action": f"Verification {v_result}", "detail": f"Core banking checks completed"},
        ]

        if fin_resolution:
            audit_log.append({"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Financial Agent", "action": "Financial resolution posted", "detail": f"JE {fin_resolution['journalEntry']}"})
            audit_log.append({"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Comms Agent", "action": "Compliant response generated", "detail": "BNM/FMOS disclosures embedded"})
        else:
            audit_log.append({"time": datetime.utcnow().strftime("%H:%M:%S"), "actor": "Comms Agent", "action": "Dispute acknowledgement dispatched", "detail": "5-day BNM SLA & FMOS disclosures sent to complainant"})
