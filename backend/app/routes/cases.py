...
from backend.app.models.case import Case, CaseRead, CaseUpdate
...
def format_case(c: Case) -> dict:
    return {
        ...
        "dueDate": c.due_date.isoformat() if (c.due_date and hasattr(c.due_date, 'isoformat')) else str(c.due_date),
...
def update_case(case_id: str, case_update: CaseUpdate, session: Session = Depends(get_session)):
    ...
    # If investigator approves / resolves case, dispatch resolution email to customer Gmail
    if case_update.status in ['PASS']:
        try:
